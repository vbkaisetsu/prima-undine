...
impl FunctionFwImpl for FlipFwImpl {
    fn call(&self, xs: &[&Tensor], u32data: &[u32], _f32data: &[f32], ys: &mut [&mut Tensor]) {
        ...
        let r = s.size() as usize / n;
        unsafe {
            let px = const_ptr!(x);
            let py = mut_ptr!(y);
            for j in 0..n {
                for i in 0..r {
                    let offset = i * n - i % skip * (n - 1);
                    *py.add(offset + j * skip) = *px.add(offset + (n - j - 1) * skip);
                }
            }
        }
    }
    ...
    fn call(
        ...
        unsafe {
            ...
            for j in 0..n {
                for i in 0..r {
                    let offset = i * n - i % skip * (n - 1);
                    *pgx.add(offset + j * skip) += *pgy.add(offset + (n - j - 1) * skip);
                }
            }
        }
    }
