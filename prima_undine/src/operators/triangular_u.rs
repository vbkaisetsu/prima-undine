...
define_operator_struct!(TriangularU, k, u32);
impl<'arg, 'dev> Operator<'arg, 'dev> for TriangularU<'dev> {
    fn name(&self) -> String {
        "TriangularU(k=".to_string() + &self.k.to_string() + ")"
    }
    ...
    fn forward(&self, x: &[&Tensor], y: &mut [&mut Tensor<'arg>]) {
        y[0].replace(self.device.triangular_u_fw(x[0], self.k));
    }

    fn backward(&self, x: &[&Tensor], y: &[&Tensor], gy: &[&Tensor], gx: &[&RefCell<Tensor>]) {
        self.device
            .triangular_u_bw(x[0], y[0], gy[0], self.k, &mut *gx[0].borrow_mut());
    }
}
